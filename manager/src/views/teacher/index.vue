<template>
  <el-card id="container">
    <el-button type="primary" plain @click="add_work"> 新增教师 </el-button>
      ...
      <template>
        <div v-for="(item,index) in workList" class="work" :key="index">
          <el-card>
            ...
            <div class="desc">{{ "学校: "+item.school }}</div>
            <div class="desc">{{ "头衔: "+item.title }}</div>
    ...
    >
      <el-form size="mini">
        ...
        <el-form-item label="头衔" label-width="100px">
          <el-input v-model="modal.title" />
        </el-form-item>
        <el-form-item label="学校" label-width="100px">
          <el-input v-model="modal.school" />
        </el-form-item>
        ...
        <el-form-item label="照片上传" label-width="100px">
          <el-upload
            class="upload-demo"
            action=""
            :file-list="modal.pic"
      ...
      <el-row :gutter="10">
        <el-col :span="16">
          <el-form size="mini">
            ...
            <el-form-item label="学校" label-width="100px">
              <el-input v-model="edit_form.school" />
            </el-form-item>
            <!-- <el-form-item label="作品上传" label-width="100px">
              <el-upload
                drag
            ...
            <el-form-item label="修改照片" label-width="100px">
              <el-upload
                class="upload-demo"
                action=""
                ...
                list-type="picture"
              >
                <el-button size="small" type="primary">点击上传</el-button> <span>{{ edit_form.pic_url?'已存在照片':'未上传照片' }}</span>
                <div slot="tip" class="el-upload__tip">只能上传jpg/png文件</div>
              </el-upload>
            </el-form-item>
            <!-- <el-form-item label="对照文本上传" label-width="100px">
...
import { uploadTeacher, getTeacherAll, updateTeacher, deleteTeacher } from '@/api/api'
export default {
  name: 'Work',
  data() {
    return {
  ...
  methods: {
    ...
    submitUpload() {
      ...
      form_data.append('school', this.modal.school)
      form_data.append('description', this.modal.description)
      uploadTeacher(form_data).then(res => {
        if(res.error_code==0){
          this.getWorks()
    ...
    submitUpdate() {
      ...
      if (this.edit_form.pic.length == 0&&this.edit_form.pic_url=="") { Message.warning('未上传照片');return}
      if (this.edit_form.name == '') {Message.warning('未填写姓名');return}
      ...
      if (!this.edit_form.school) { Message.warning('未选择学校'); return }
      if (this.edit_form.title == '') { Message.warning('未填写头衔'); return }
      const form_data = new FormData()
      ...
      form_data.append('school', this.edit_form.school)
      ...
      form_data.append('name', this.edit_form.name)
      // for (var p of form_data) {
      //   console.log(p)
      // }
      updateTeacher(form_data).then(res => {
        if(res.error_code==0){
          this.getWorks()
    ...
    delteWork() {
      deleteTeacher({ id: this.edit_form.work_id }).then(res => {
        if(res.error_code==0){
          this.getWorks()
